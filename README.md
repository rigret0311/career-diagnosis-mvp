...
公開版として、Sitesで動くブラウザ完結版も同じリポジトリに含んでいます。公開版は段階式フォーム、市場価値レポート、端末内の診断履歴、テキスト保存、印刷・PDF、共有、相談内容の作成に対応します。

## 2つの動かし方

- `./start.sh` / `start.bat`: Flask版。JSON保存とSMTPメールを含むローカル運用向け
- `npm run dev`: Sites版。公開サイトと同じ画面をローカルで確認するためのブラウザ完結版

公開版は個人情報をサーバーへ保存せず、ブラウザのローカルストレージだけを使います。実際の相談受付メールを有効にする場合は、Sitesの環境変数 `NEXT_PUBLIC_CONTACT_EMAIL` に公開用メールアドレスを設定してください。

## 公開版の起動方法

Node.js 20以降が必要です。初回だけ依存関係をインストールします。
